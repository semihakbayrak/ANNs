%Regression with Mixture of Experts
function MoE(H)
...
%Initial parameters
v_list = (rand(H,1)-0.5)/50;
v0_list = (rand(H,1)-0.5)/50;
...
for epoch=1:numofEpochs
    for t=1:length(x_list)
        ...
        w_list = v_list*x_list(t) + v0_list;
        y = dot(w_list,g_list);
        delta_v_list = nu2*(r_list(t)-y)*g_list*x_list(t);
        delta_v0_list = nu2*(r_list(t)-y)*g_list;
        ...
        y_list(t) = y;
        v_list = v_list + delta_v_list;
        v0_list = v0_list + delta_v0_list;
...
for t=1:length(x_list)
    ...
    w_train = v_list*x_list(t) + v0_list;
    ytrain(t) = dot(w_train,gtrain);
end
figure
...
%w_h, training data and overall fit
ytrain = zeros(length(x_list),1);
ptrain = zeros(H,1);
for t=1:length(x_list)
    for h=1:H
...
w_h = zeros(length(x_list),H);
for h=1:H
   for i=1:length(x_list)    
       w_h(i,h) = v_list(h)*x_list(i) + v0_list(h);
   end
end
for h=1:H
    plot(x_list,w_h(:,h),'color',color_list(h,:),'LineWidth',1.2)
    hold on
end
title('w_h plots with training data and overall fit')
xlabel('x')
ylabel('y')
...
for t=1:length(xval)
    ...
    gval = pval;
    gval = gval/sum(gval);
    w_val = v_list*x_list(t) + v0_list;
    yval(t) = dot(w_val,gval);
end
